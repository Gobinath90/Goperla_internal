...
test.describe('Signup Page Tests', () => {
  ...
  test.describe('Register Page Feild Validation', () => {
    ...
    test('SignUp-0019/20/21/22/23/24/25: Verify the "Submit" button without entering any OTP on the OTP Verification Page [Smoke]', async ({ page, baseURL }) => {
      ...
      await page.waitForURL(`${baseURL}/verification`);
      ...
      await page.getByRole('button', { name: 'Submit' }).click();
      await verifyTextVisibility(page, [
        'Company Name is required', 'Street Address 1 is required', 'State is required', 'City is required', 'Zip Code is required',
        'Phone Number is required', 'Mobile Number is required', 'Domain is required']);
      await expect(page.locator('#city-helper-text')).toContainText('City is required');
    ...
    test('SignUp-0026: Verify User Login and UI Elements on Select Plan Page [Smoke]', async ({ page, baseURL }) => {
      ...
      await verifyTextVisibility(page, ['Welcome User', 'Entity user super admin', 'Pricing and Benefits', 'Locations', 'Users', 'Additional Location Per Month', 'Storage',
        ...
        'Basic', 'Essential']);
      await expect(page.getByRole('heading', { name: 'Transparent pricing with no' })).toBeVisible();
    ...
    test.skip('SignUp-0012: Verify Successful Sign-up with Email and OTP Validation [Smoke]', async ({ page }) => {
      ...
      await verifyVisibility(page.getByText('Verified successfully!'), 'Verifed successfully message');

      // // Step 6: Verify various tabs and headings are visible
      // await verifyEleVisibility(page, [selectPlan.tab.selectPlan, 'image Make Payment', 'image Create Company'], 'tab');
    ...
    test.skip('SignUp-0012aaaaa: Verify Successful Sign-up with Email and OTP Validation [Smoke]', async () => {
      console.log(`Random email generated: ${dynamicEmail}`);
      // get value
      await performLogin(loginPage, dynamicEmail, 'Test@123', 'Credentials with unicode special characters');
    });
  });
