...
const CREDENTIALS = {
  password: 'Test@123',
  baseEmail: 'gobinath.ravichandran',
...
class AuthenticationHelper {
  ...
  private generateDynamicEmail(): string {
    return `${CREDENTIALS.baseEmail}+${Date.now()}@${CREDENTIALS.domain}`;
  }
  ...
  async handleMicrosoftLogin() {
    ...
    await outlookPage.locator(SELECTORS.password).fill(process.env.M365_PASSWORD);
  ...
  async getVerificationCode(outlookPage) {
    ...
    const emailContent = await outlookPage.locator(SELECTORS.otpContent).textContent();
...
test.describe('Authentication Flow', () => {
  test('complete registration with email verification', async ({ context, page, baseURL }) => {
